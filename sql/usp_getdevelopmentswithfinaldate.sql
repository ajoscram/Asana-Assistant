...
CREATE PROCEDURE usp_getdevelopmentswithfinaldate
	@idtask bigint,
	@finaldate date
...
BEGIN
	...
	ELSE IF @idtask IS NOT NULL AND @finaldate IS NOT NULL
		...
		WHERE RELDEVELOPMENTTASK.IDtask = @idtask AND DEVELOPMENT.datecreated <= CAST(@finaldate  AS DATE)
	ELSE
